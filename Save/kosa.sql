...
insert into temp01(id,job) values(200, 'SALSES');
...
-- 부서별 평균 급여를 구하세요.
-- select deptno , avg(sal) from emp; --depno는 여러건이고 avg(sal)은 한건 그럼 avg(sal)을 어디다 배치할껀데 그래서 에러
...
문제 풀어 보세요
*/
select department_id, MAX(salary) minsalary

select department_id, MIN(salary), max(salary) , max(salary) -min(salary) as difference
from employees
...
2. multi row subquery  : 실행 결과가 단일컬럼에 여러개의 값
...
문법) 
    1. 괄호안에 있어야 한다. (쿼리문)
    2. 단일 컬럼을 가져야한다 ( 멀티컬럼을 쓰면 유추하기 힘들어짐0
