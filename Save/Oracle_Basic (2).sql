...
WHERE TOTAL_SOLD = (SELECT MAX(SUM(QUANTITY)) FROM ORDER_DETAILS GROUP BY PRODUCT_ID);
...
WHERE ORDER_COUNT = (SELECT MAX(COUNT(ORDER_ID)) FROM ORDERS GROUP BY USER_ID);
...
--글쓰기
--PK   not null , unique , index (where empno=7888)
...
values((select count(boardid)+1 from board),'방가1');

insert into board(boardid, title) 
values((select count(boardid)+1 from board),'방가2');

insert into board(boardid, title) 
values((select count(boardid)+1 from board),'방가3');
...
insert into board(boardid, title) 
values((select count(boardid)+1 from board),'방가4');
...
select board_num.currval from dual; --현재 채번한 마지막 번호 확인
...
순번을 생성 (테이블 종속)
Ms-sql
create table board(boardnum int identiy(1,1),title)
...
--rownum
--1. 의사컬럼 : 실제 물리적으로 존재하는 컬럼은 아니고 논리적 존재 
--create table 생성안되요 
...
from (
      ...
      ) n  where  num <= 5; --대용량 데이터 페이징 처리 (TODAY POINT)
...
1번페이지 > 1~10   > num between 1 and 10
2번페이지 > 11~20  > num between 11 and 20
...
☞ Guidelines  
이미 존재한다면 다시 생성한다. 
Base Table 유무에 관계없이 VIEW을 만든다. 
기본 테이블이 존재할 경우에만 VIEW를 생성한다. 
VIEW 의 이름 
Subquery 를 통해 선택된 값에 대한 Column명이 된다. 
SELECT 문장을 기술한다. 
VIEW 에 의해 액세스 될 수 있는 행만이 입력,갱신될 수 있다.  
CHECK OPTON 제약 조건에 대해 지정된 이름이다. 
이VIEW 에서 DML이 수행될 수 없게 한다. 


*/
--View 가상 테이블 (물리적 테이블 볼수 있는 쿼리문을 가지고 있는 객체)
--사용 방법은 테이블 처럼 ^^
...
--select comm  from view001; view  가 볼수 있는 데이터만 처리 가능
...
--in line view > view 객체로 만들어서 join 하는 것이

-- 직종별 평균급여를 볼 수있는 view 생성
-- view_003
...
--view 통해서 볼수 있는 데이터 (select ,insert ,update, delete)가능
update v_emp
set job='IT';
...
select deptno, case when deptno=10 then 1 else 0 end dept_10, --컬럼으로 데이터 판단
...
select deptno, sum(case when deptno=10 then 1 else 0 end) as dept_10, --컬럼으로 데이터 판단
...
select  sum(case when deptno=10 then 1 else 0 end) as dept_10, --컬럼으로 데이터 판단
        sum(case when deptno=20 then 1 else 0 end) as dept_20,
        sum(case when deptno=30 then 1 else 0 end) as dept_30
from emp;
